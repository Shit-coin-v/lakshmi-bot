...
- Перед выполнением каждой задачи агент обязан прочитать текущий журнал docs/AGENT_WORKLOG.md, чтобы понимать контекст последних действий и не дублировать проверки/правки.
- Любые тесты/проверки/аудиты фиксировать в docs/AGENT_WORKLOG.md, даже если файлы не менялись; примеры: `ruff check`, `mypy`, `pytest`, `python -m compileall`, `docker compose -f infra/docker/docker-compose.yml config`, security/audit-проверки. Если команда недоступна или завершилась ошибкой — это тоже фиксируется. В записи указывать “Какие файлы изменены: без изменений”, если это был только запуск проверок/аудита.
...
- Формат каждой записи (минимум):
  ...
  - Какие файлы изменены (список путей; если изменений не было — “без изменений”)
  - Какие проверки/команды запускались и результат (успех/ошибка)

## Требования к фиксации git-состояния в отчётах (обязательно)

При выполнении задач, связанных с изменением файлов и последующим коммитом, агент обязан соблюдать следующий порядок и отражать его в отчёте:

1) **Перед коммитом** обязательно показать:
   - `git diff` — с ожидаемыми изменениями
   - `git status -sb` — с изменёнными файлами

2) **После коммита** обязательно повторно показать:
   - `git diff` — должен быть пустым
   - `git status -sb` — рабочее дерево должно быть чистым

Запрещено:
- Показывать `git diff` и `git status` только до коммита.
- Завершать отчёт состоянием, в котором файлы остаются помеченными как `M`.
- Оставлять неоднозначность, закоммичены ли изменения или нет.

Отчёт считается корректным **только если финальное состояние репозитория явно зафиксировано как чистое**.

Примечание:
- Если по итогам задачи **изменений в коде нет** и коммит не выполнялся, агент обязан:
  - явно указать, что изменений нет,
  - показать `git diff` (пустой),
  - показать `git status -sb` (чистый),
  - **не выполнять коммит** и не создавать PR.

Запрещено:
- Делать коммит, если `git diff` пустой.
- Создавать PR без фактических изменений в коде.
