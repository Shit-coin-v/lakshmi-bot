...
def _check_picker(user_id: int) -> bool:
    return user_id in PICKER_ALLOWED_TG_IDS
...
async def cmd_orders(message: Message):
    if not _check_picker(message.from_user.id):
...
async def orders_back(callback: CallbackQuery):
    if not _check_picker(callback.from_user.id):
