...
def onec_customer_sync(request):
    """
    1С <-> Бот: синхронизация/поиск клиента ТОЛЬКО по telegram_id.

    Режимы:
      • READ-ONLY: прислан только telegram_id — возвращаем карточку без изменений.
      • WRITE: если присланы bonus_balance / referrer_telegram_id / one_c_guid — создаём/обновляем.
        Дата не обязательна; если нет — используем "сейчас".
    """
    ...
    write_mode = (bonus_balance is not None) or bool(referrer_tid) or bool(one_c_guid)
    ...
    try:
        m = OneCClientMap.objects.filter(user=user).first()
        guid_for_resp = getattr(m, "one_c_guid", None) or (one_c_guid or None)
    except Exception:
        guid_for_resp = one_c_guid or None
